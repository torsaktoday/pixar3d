...
import { Copy, Check, Terminal, FileText, ShieldAlert, List, AlertTriangle, CheckCircle, RefreshCw, ArrowRight, FileCheck, Clapperboard, Wand2, Image as ImageIcon } from 'lucide-react';
import { AnalysisResult, AnalysisMode, ProductionGuide, ProductionScene } from '../types';
import { rewriteScript, generateProductionGuide } from '../services/geminiService';
        ...
        <div className="w-full max-w-3xl mx-auto mt-8 space-y-8 animate-fade-in-up">
           ...
           {fixedScript && (
             <div className={`bg-slate-800 rounded-xl border overflow-hidden shadow-2xl animate-fade-in ${usedOriginal ? 'border-purple-500/30' : 'border-green-500/30'}`}>
                ...
                <div className="p-6">
                  {/* Script Preview */}
                   <div className="mb-8 p-4 bg-slate-900/50 rounded-lg border border-slate-700 text-slate-300 text-sm max-h-40 overflow-y-auto whitespace-pre-wrap">
                      {fixedScript}
                   </div>
